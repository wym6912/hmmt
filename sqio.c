...
WriteSeq(FILE *outf, int outform, char *seq, SQINFO *sqinfo)
{
  ...
  switch (outform) {
    ...
    case kGenBank:
      ...
      if (sqinfo->flags & SQINFO_FREE)
	fprintf(outf, "%s", sqinfo->free);
      fprintf(outf,"ORIGIN      \n");
      spacer = 11;
      numline = 1;
      strcpy(endstr, "\n//");
    ...
    case kPearson:
      if (sqinfo->flags & 
	  (SQINFO_WGT | SQINFO_ID | SQINFO_ACC | SQINFO_START | SQINFO_STOP | SQINFO_OLEN))
	{
				/* special FASTA */
		  ...
		  int               *ret_num)
{
  ...
  if (fformat == kSelex || fformat == kMSF)
    {
...
GSIOpen(char *gsifile)
{
  ...
  if ((gsi->gsifp = fopen(gsifile, "rb")) == NULL)
    { squid_errno = SQERR_NOFILE; return NULL; }
